...
// greeting함수는 processUserInput함수의 인자로 전달되는 콜백함수
...
function fetchUserData(userId, callback) {
  ...
  setTimeout(() => {
    ...
    const userData = {
      id: userId,
      name: '안찬숙',
      email: 'ckstnr99@naver.com'
    }
    callback(userData);
  }, 3000); // 3초 후 사용자 데이터를 전달하는 콜백함수를 호출
};

// fetchUserData로 전달되는 인자함수
// : 익명함수로써, 해당 인자의 역할 제외하고는 호출되지 않음.
...
// EX)
// A함수가 33번째 줄에서 setTimeout설정으로 3초 뒤에 실행
// B함수가 40번째 줄에서 일반 함수로써 호출
// >> 해당 파일 실행 시 A / B 중 먼저 실행되는 코드 -> (B)

//! 4. 비동기 프로그래밍으로써 콜백함수의 단점
// 콜백함수는 비동기 작업이 중첩될수록 코드의 인자로 전달되는 코드의 중복이 늘어남

//? 콜백지옥(callback hell)
// : 중첩된 콜백함수들이 여러 단계에 걸쳐서 깊게 중복되는 문제점
// - 가독성 저하
// - 디버깅 어려움
// - 유지 관리 어려움
...
loginUser('example@naver.com', 'password', function(error, user) {
  ...
  //? 익명 함수 실행 완료 전에 checkAccess함수 실행 시작
