//! 자바스크립트의 비동기 프로그래밍 - Promise(프로미스)
...
//? resolve(value): 프로미스를 이행상태로 변경, 결과값 value를 반환
...
.finally(() => {
  console.log('Operaition completed'); // 성공/실패 상관없이 수행
});

// 계단식 전달 방법을 사용한 콜백함수와 달리 Promise는 then과 catch블럭을 사용하여 코드의 가독성을 증가
...
function fetchData() {
  return new Promise((resolve, reject) => {
    setTimeout(() => resolve('Data Fetched'), 3000); // 3초
  }); 
};
