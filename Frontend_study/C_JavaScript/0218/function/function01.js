...
cars.forEach((car) => { // 하나씩 요소를 뽑아내니 단수 형태로 이름 설정 cars -> car
  console.log(car); // 각 요소가 반복될 때마다 실행할 함수
})

// map(): 배열의 각 요소에 대해 함수를 적용하고 새로운 배열을 생성하여 반환
// 문자열데이터.toUpperCase(): 대문자 변환(영문자만)
let carsUpperCase = cars.map(car => car.toUpperCase()); // 인자값 하나일 땐 괄호 사용 안 해도 됨
...
// 배열명.reduce((accumulatorm currentValue,
// currentIndex, array) => {
  // 함수 로직
// }, initialValue);
...
let numbers = [1, 2, 3, 4];
// 콜백함수 지정 시
// : 단일 매개변수 또는 단일 계산식에서 괄호 지정 시 계산의 오류 발생 가능성이 증가
...
console.log(combinedCarName); // audi, bmw, hyundai, volvo
...
numberArray.sort((a, b) => { // 풀이할 것 없이 그냥 sort 이용한 오름차순 그 자체의 식인듯!
  // if (a > b) return 1;
  // if (a === b) return 0;
  // if (a < b) return -1;
  return a - b;
...
console.log(firstOverFiveIndex); // 3: index번호를 반환