...
//! 생성자 함수를 사용한 객체 생성 (cf.붕어빵 만들기)
// 생성자 함수를 사용할 경우 코드를 재사용하여 같은 구조의 객체를 여러개 생성 가능
...
// 생성자 함수를 사용한 객체 생성
let person1 = new Person('찬숙', '안', 18, 'female', ['exercise', 'develop']);
let person2 = new Person('피식', '대학', 28, 'female', ['exercise', 'develop']);
// >> 생성자 함수로 만들어진 객체 내의 this는 new 키워드를 통해 만들어진 객체 그 자체
person1.greeting(); // Hello, 찬숙
person2.greeting(); // Hello, 피식
...
// : 클래스 내부에서 메서드 정의 시 function 키워드를 사용하지 않음!
...
class Person2 {
  ...
  }; // 이 문장을 밖에다 작성해서 ERROR 났었음!
};

let personA = new Person2('이용주', 18, 'high-school');
let personB = new Person2('정재형', 15, 'middle-school');
personA.greet(); // 안녕하세요 이용주님
personB.greet(); // 안녕하세요 정재형님
