...
? 2. 구문 오류
: 괄호의 짝이 다름, 문자열을 열었는데 닫지 않음 등
구문 자체의 오류가 있으면 웹 브라우저가 코드를 분석조차 하지 않음 >> 실행 안됨!
...
// console.log('런타임 오류 발생 이후의 코드는 실행되지 않음');
...
document.addEventListener('DOMContentLoaded', () => {
  // HTML에 h1요소가 존재하지 않음
  // : ReferenceError 발생
  // : HTML 웹문서(document)에서 참조할 h1요소가 존재하지 않음

  //? else문으로 코드의 예외는 피했지만
  // : 예외를 처리하지 않았기 때문에 해당 예외 이후의 코드가 실행되지 않는다.
...
try {
  ...
  // : 객체 자체가 존재하지 않음
  willExcept.byeBye(); 
  // : try구문 안에서 예외가 발생하면 더 이상 try구문을 진행하지 않는다.
  // : catch구문을 실행
  console.log('try구문의 마지막 줄');
} catch (exception) {
  console.log('catch구문의 마지막 줄');
} finally {
  console.log('finally구문의 마지막 줄');
};
...
sample();
// : sample함수의 실행 시 A, B, C의 위치가 모두 출력
// : finally구문 앞에 return이 있더라도 finally구문은 반드시! 실행!한다는 특성