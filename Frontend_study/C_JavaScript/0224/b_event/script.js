// 이벤트에 필요한 개념 정리 (cf=> html, style.css)

//! 이벤트 객체
// : 이벤트 핸들러가 호출될 때, 브라우저는 자동으로 이벤트 객체를 생성하여 이벤트 핸들러에 전달
// : 이벤트와 관련된 여러 정보가 포함되어 전달
// 예) 마우스 이벤트 - 마우스의 위치 정보 전달
// 예) 키보드 이벤트 - 어떤 키가 눌러졌는지에 대한 정보 전달
...
function bgChange(e) {
  ...
  // document.body.style - 웹 문서의 전체 스타일 설정

  // e.target (해당 이벤트가 발생한 그 객체에서만 스타일을 지정)
  // : div를 클릭할 경우 해당 이벤트 리스너가 실행되는 주체(e.target)는 div 요소 자체
