...
- 시작버튼 > 자격 증명 관리자
  ...
  > git / github / sourcetree 에서 타인의 아이디로 설정 되어 있는 경우 삭제(제거)
...
- 사용자 이름과 이메일 설정
  ...
  ? 사용자 이름 설정
  git config --global user.name 깃허브아이디(본인)
  ex) git config --global user.name devgiants75

  ? 사용자 이메일 주소 설정
  git config --global user.email 깃허브이메일(본인)
...
2. staging area: 커밋하기 전에 파일의 변경사항을 준비하는 장소,
                  기존의 로컬 스토리지와의 변경사항을 검토하고 커밋할 항목을 결정
...
? git add .
  ...
  : git add 파일명 / git add . (모든 변경사항 추가) - 추천

? git commit -m "커밋 메시지"
  : 스테이징 영역의 변경사항을 (로컬)저장소의 기록으로 작성
  : 강사님 version - 20240128 isa "grid layout"
...
echo "# Frontend_study" >> README.md
>> README.md 파일을 생성
  (# Frontend_study 텍스트를 추가)
...
git remote add origin https://github.com/Anchansook/Frontend_study.git
>> 리모트 저장소를 추가 

git push -u origin main
>> 로컬 리포지토리 main 브랜치를 원격 리포지토리에 푸시
>> -u: 추후 git push나 git pull을 진행할 때 브랜치 이름을 생략 가능하도록 설정
