...
: (위)사용자 변수에서 변수 Path 더블 클릭
    ...
    - 파일 탐색기 > c드라이브 > 사용자(Users) > 자신의 컴퓨터 이름(ITPS) 
    > AppData > Roaming > npm 경로를 복사
    (npm )
    : C:\Users\ITPS\AppData\Roaming\npm

: (아래)시스템 변수 Path 설정
    파일 탐색기 > c드라이브 > ProgramFiles > nodejs 경로를 복사
...
? 프로젝트 디렉토리 생성
  ...
  - 강사님 >> C:\프론트엔드-이승아\frontend-1223-lsa  
  - ME >> C:\frontend-1223-asc
  >> mkdir D_TypeScript
  >> cd D_TypeScript
