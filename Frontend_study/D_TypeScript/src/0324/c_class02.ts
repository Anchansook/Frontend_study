...
// - 반환값이 없는 함수, 자신을 new 키워드와 함께 
// 호출하는 대상(클래스)의 새로운 인스턴스를 암시적으로 반환
// - 필수적인 요소가 아님!, 생성자가 없는 경우 자동으로 빈 생성자를 추가
...
let person1 = new Person('이용주', 29);
console.log(person1.name); // 이용주
console.log(person1.describe()); // 이용주 is 29 years old

let person2 = new Person('정재형', 30)
console.log(person2.name); // 정재형
console.log(person2.describe()); // 정재형 is 29 years old

person1.name = '김민수';
// : 인스턴스 속성 값을 클래스를 통하지 않고 속성에 접근하여 변경이 가능
console.log(person1.name); // 김민수
console.log(person2.name); // 정재형
...
class Dog {
  ...
  // : 멤버 변수의 초기값으로 사용되지 않는 로직상 
  // 데이터 저장이 필요없는 데이터를 매개변수로 가질 수 있다.
...
// 화살표 함수의 경우 해당 함수 내에서 this를 만들어내지 못하기 때문에 
// 해당 함수가 선언된 범위의 this를 찾아서 사용
// : 코드의 논리이해가 필수적
