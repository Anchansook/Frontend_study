...
// : 변수명 뒤에 콜론을 이용하여 JS코드에 타입을 정의하는 방식

// 타입(Type)
//? 1. 기본타입(원시타입: string, number, boolean 등)
// : 변수명 뒤에 콜론을 첨부하고 타입을 명시
// : typeof연산자를 사용했을 때 나타나는 변환값과 동일한 이름으로 명시
...
// - 배열과의 차이점
//  : 튜플의 경우 Array<> 제네릭 타입 지정 방식 사용 안함!

// 타입을 지정할 때 
// : 원소의 타입을 순서대로 작성

const nameAndHeight: [string, number] = ['이용주', 190];
// const tuple: [number, string, number] = [1, 2, '3']; - Error
// : 튜플은 각 원소의 타입과 순서를 지정

// Array객체의 메서드로 조작되는 요소의 변화는 가능
nameAndHeight.push(1);
console.log(nameAndHeight); // [ '이용주', 190, 1 ]
...
// : null과 undefined에 자기 자신의 타입 그리고 void타입 외의
// 타입에 할당하는 경우 에러 발생

// JS
// - null과 undefined변수에 다른 타입의 값이 할당 가능

// TS
// - null타입의 변수에는 'null'값만!
// - undefined타입의 변수에는 'undefined'값만!
...
// any타입의 경우 사전에 타입 오류를 계산할 수 없다.
// >> 예상치 못한 에러 발생 가능성이 높음(사용 권장 안 함)
...
//? 8. bigint (es2020에 만들어짐)
// : 큰 정수를 다루기 위한 원시 타입
// : Bigint함수를 사용하여 bigint값을 생성

// const oneHundred: bigint = Bigint(100);
// 오류(lib 컴파일 옵션을 'es2020' 이상으로 변경)

//? 9. symbol
// : 전역적으로 고유한 참조값을 생성하는 데 사용하는 타입
// : 유일하고 변경 불가능한 값으로 생성
// Symbol()함수로 생성

const referrence = Symbol('name');
// : referrence변수에 name이라는 설명을 가진 심볼(상징)을 생성
