...
number = 123.456;
anyNumber = 123.789;
console.log(number.toFixed(2)); // 소수점 2번째 자리까지 반올림 - 123.46
console.log(anyNumber.toFixed(2)); // 123.79

number = '전화번호';
anyNumber = '전화번호';
// console.log(number.toFixed(2)); - Error
// : any타입의 변수로 명시하지 않은 경우 값을 할당할 때마다 해당 값의 타입의 변수로 재지정

// console.log(anyNumber.toFixed(2)); - Error (문자열에 숫자메서드를 사용할 수 없음)

//! 2. 타입 별칭(Type Alias) ** 중요!!
...
// user.height = 180; - Error - 읽기 전용 속성이므로 'height'에 할당할 수 없습니다.
...
// 3. 함수 타입 별칭
// : User타입인 매개변수를 받아 boolean타입의 반환값을 생성하는 함수
type User = {
  id: string;
  name: string;
};
...
// 4. 함수 사용
console.log(greet('이용주')); // Hello, 이용주
console.log(farewell('김민수')); // Goodbye, 김민수
